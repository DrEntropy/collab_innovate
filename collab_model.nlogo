...
- If you set the success threshold low (e.g. 1) with a high number for T-max, the whole network will settle down to a single idea. There is also a regime where the 'idea' will remain stable for a while and then jump to another stable state. Color by 'idea' to see this effect.  Try setting p so that the observed success probability is close to but not 1.
